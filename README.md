...
Some components (e.g., labeling, logging) require credentials.

* **Weights & Biases (W\&B)** for logging.
* **LLM/NLI backend** for labeling (default: OpenAI-compatible endpoint).
...
### 2) Environment variables for auxiliary scripts

Edit `./semantic-entropy-probes/semantic_uncertainty/generate_answers.py` (use `/` in paths):
...
> **Security:** Prefer environment variables; never commit secrets.

---

## Step 1 — Generate Multi-turn CoT Continuations
...
The script reads `config.json`, generates **multi-turn continuations** and records **hidden states**.
...
```json
{
  ...
  "sampling": { "strategy": "random", "n": 1000, "seed": 42 },
...
## Step 2 — Compute SpikeScore / SEP

Compute metrics from hidden states:
...
| `--probe_path`  | str  | —                      | Path to trained probe `.pkl` (for SEP/SAPLMA) |
| `--token_key`   | str  | `last_token_embedding` | Hidden-state key                              |
| `--layer_range` | str  | `None`                 | Inclusive layer range, e.g., `13,21`          |

The script writes metric fields to each JSON, e.g.,
...
> **Note.** **SpikeScore** is training-free; probe-based baselines require `--probe_path`.

---

## Step 3 — Evaluate Separability and AUC

Aggregate metrics and labels, then compute **separability** (mixture-domain) and **AUC** (leave-one-out):
...
* Recursively scans for `.json`/`.pkl`.
* Produces mixture-domain **separability** statistics and leave-one-out **AUC**, plus plots under `--report_dir`.

> **Interpretation.** Mixture-domain separability corresponds to the Mann–Whitney probability over a mixed pool (our primary statistic with a proven lower bound); AUCs in the main tables are per-domain (leave-one-out) and then averaged.

---

## Optional — Train a Probe (for baselines)

We include an accelerated pipeline adapted from public implementations for training probe-based baselines (e.g., **SEP**, **SAPLMA**).
...
### Generate training data (labels + hidden states)
...
* `model_path` *(str)*: Local/HF path to the model checkpoint
* `dataset_path` *(str)*: Input dataset (`.json`/`.jsonl`)
* `output_dir` *(str)*: Destination for JSON/PKL pairs
* `sampling.strategy` *(str)*: e.g., `random`
* `sampling.n` *(int)*: Number of items
* `sampling.seed` *(int)*: RNG seed
* `enable_thinking` *(bool)*: Enable auxiliary “thinking” if supported
* `strategy` *(str)*: Generation strategy (e.g., `progressive`)
* `max_steps` *(int)*: Max continuation steps
* `temperature`, `top_p`: Decoding parameters
* `enable_ppl_detection` *(bool)*: Enable perplexity checks
* `generation_timeout` *(int, sec)*: Per-item timeout
* `model_max_tokens` *(int)*: Context budget
...
Each example yields a **JSON/PKL pair** under `./outputs/...`:

```
outputs/
  1da17f67a300.json   # metadata, CoT steps, labels, metrics
  1da17f67a300.pkl    # hidden states (layer × token tensors)
```

**JSON (illustrative)**

* `id`, `question`, `cot_steps`
* `nil_label` / reference labels
* `metrics.spike_score`, `metrics.sep_score`, `metrics.saplma_prob`, …

---

## Tips & Troubleshooting

* **Precision & Speed** — Prefer **FP16 non-quantized** checkpoints for faithful hidden states and fast extraction.
* **OOM / Memory** — Reduce `max_new_tokens`, sequence length, or batch size; use sharding / CPU offload.
* **Probe Alignment** — Match `token_key` and `layer_range` between probe training and inference.
* **W\&B** — First run may prompt `wandb login`. Ensure `WANDB_ENT` / `WANDB_API_KEY`.
...
> This README explains how to **replicate the experiments** from our paper using **Chain-of-Thought (CoT) path perturbation**. The pipeline generates model responses **and** hidden states, then computes collapse-style metrics (e.g., **SEP**, **RS**) for hallucination detection.
...
* [Setup & Installation](#setup--installation)
* [Credential Configuration](#credential-configuration)
* [Step 1 — Run CoT Generation](#step-1--run-cot-generation)
