...
class CustomLLM:
    ...
    def invoke_with_metrics(self, prompt: str) -> Tuple[str, Dict[str, Any]]:
        """生成文本并计算条件PPL和其他指标，支持超时"""
        ...
        if generation_thread.is_alive():
            ...
            return timeout_text, timeout_metrics
...
class CoTReasoner:
    ...
    def _get_safe_token_limit(self) -> int:
        ...
        # 3. 检查是否为合理值（小于10万）
        if tokenizer_limit and tokenizer_limit < 200000:
    ...
    def process_question(self, data_item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.config.get("enable_ppl_detection", True) and hasattr(self.llm, 'invoke_with_metrics'):
            qa_answer, initial_metrics = self.llm.invoke_with_metrics(initial_prompt)
        ...
        for step in range(1, self.config["max_steps"] + 1):
            ...
            try:
                # 使用新的invoke方法
                if self.config.get("enable_ppl_detection", True) and hasattr(self.llm, 'invoke_with_metrics'):
                    cot_answer, step_metrics = self.llm.invoke_with_metrics(conversation)
