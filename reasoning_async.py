...
class CustomLLM:
    ...
    def __init__(self, model, tokenizer, config):
        ...
        self.max_new_tokens = config.get("max_generation_tokens", 2048)
...
class CoTReasoner:
    ...
    def __init__(self, config_path: str = "config.json"):
        ...
        from llm_async_engine import AsyncLLMEngine  # 或上面直接定义即可
        self.llm_async_engine = AsyncLLMEngine(self.config["model_path"], llm_config, num_workers=2)
    ...
    def ask_llm_async(self, prompt, task_id):
        self.llm_async_engine.submit(prompt, task_id)

    def wait_llm_result(self, task_id, timeout=None):
        while True:
            result = self.llm_async_engine.get_result(timeout=timeout)
    ...
    def ask_and_wait(self, prompt, task_tag, timeout=None):
        task_id = f"{task_tag}_{datetime.datetime.now().timestamp()}"
        self.ask_llm_async(prompt, task_id)
        result = self.wait_llm_result(task_id, timeout=timeout)
        output = result.get('output', '') or "[Error]"
        metrics = result.get('metrics', {}) or {}
        hidden = result.get('hidden', (None, None, None)) or (None, None, None)
        return output.strip(), metrics, hidden
    ...
    def process_question(self, data_item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if self.config.get("enable_ppl_detection", True) and hasattr(self.llm, 'invoke_with_metrics'):
            qa_answer, initial_metrics, initial_hidden = self.ask_and_wait(initial_prompt, f"{data_item.get('id', 'unknown')}_step0")
        ...
        for step in range(1, self.config["max_steps"] + 1):
            ...
            try:
                # 使用新的invoke方法
                if self.config.get("enable_ppl_detection", True) and hasattr(self.llm, 'invoke_with_metrics'):
                    cot_answer, step_metrics, step_hidden = self.ask_and_wait(conversation, f"{data_item.get('id', 'unknown')}_step{step}")

                    cot_answer = cot_answer.strip()
                # if self.config.get("enable_ppl_detection", True):
                #     task_id = f"{data_item.get('id', 'unknown')}_step{step}"
                #     self.ask_llm_async(conversation, task_id)
                #     result = self.wait_llm_result(task_id)
                #     cot_answer = result['output'].strip() if result['output'] else "[Error]"
                #     step_metrics = result['metrics'] or {}
                #     step_hidden = result['hidden'] or (None, None, None)
                else:
