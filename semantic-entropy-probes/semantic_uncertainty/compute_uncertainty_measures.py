...
def main(args):
    ...
    if args.compute_predictive_entropy:
        ...
        elif args.entailment_model.startswith('llm_gpt-'):  # 新增：自动支持所有OpenAI GPT
            from uncertainty.uncertainty_measures.semantic_entropy import EntailmentGenericLLM
            model_name = args.entailment_model[len("llm_"):]  # 去掉前缀
            entailment_model = EntailmentGenericLLM(model_name, args.entailment_cache_id, args.entailment_cache_only)
        elif 'llama' in args.entailment_model.lower():
            entailment_model = EntailmentLlama(args.entailment_cache_id, args.entailment_cache_only,
                                               args.entailment_model)
        else:
            raise ValueError(f"Unknown entailment_model: {args.entailment_model}")
    ...
    for idx, tid in enumerate(validation_generations):
        ...
        if args.compute_predictive_entropy:
            ...
            semantic_ids = get_semantic_ids(
                responses, model=entailment_model,
                strict_entailment=args.strict_entailment, example=example,
                max_workers=getattr(args, 'max_workers_for_entailment', 20))
