...
class EntailmentGenericLLM(EntailmentLLM):
    def __init__(self, model_name, entailment_cache_id, entailment_cache_only):
        super().__init__(entailment_cache_id, entailment_cache_only)
        self.name = model_name   # 比如 'gpt-4o', 'gpt-4o-mini'，全靠外部传进来
...
def get_semantic_ids(strings_list, model, strict_entailment=False, example=None, max_workers=20):
    """
    Group list of predictions into semantic meaning.

    Args:
        strings_list: list of generated answers
        model: entailment model, 需要有 check_implication 方法
        strict_entailment: 是否严格（双向都为entailment才算同义）
        example: 当前样本信息
        max_workers: 最大并发线程数
    Returns:
        semantic_set_ids: 每个字符串的语义聚类ID（整数列表）
    """
    ...
    for i, string1 in enumerate(strings_list):
        if semantic_set_ids[i] == -1:
            ...
            # 用并发判断 string1 与其后每个 string 是否等价
            tasks = []
            idxs = []
            for j in range(i+1, len(strings_list)):
                if semantic_set_ids[j] == -1:
                    tasks.append((string1, strings_list[j]))
                    idxs.append(j)

            results = []
            if tasks:
                with ThreadPoolExecutor(max_workers=max_workers) as executor:
                    futures = [executor.submit(are_equivalent, t[0], t[1]) for t in tasks]
                    for future in futures:
                        try:
                            ...
                            import logging
                            logging.warning(f"Entailment error: {e}")
