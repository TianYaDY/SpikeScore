...
def get_parser(stages=['generate', 'compute']):
    ...
    parser.add_argument('--max_workers_for_entailment', type=int, default=40,
                        help="Number of parallel entailment threads")
...
def init_model(args):
    ...
    if os.path.exists(mn):
        ...
        model = HuggingfaceModel(
            mn, stop_sequences='default',
            max_new_tokens=args.model_max_new_tokens)
    elif any(x in mn.lower() for x in ['llama', 'falcon', 'mistral', 'phi']):
        model = HuggingfaceModel(
            mn, stop_sequences='default',
            max_new_tokens=args.model_max_new_tokens)
    else:
...
def get_metric(metric):
    if metric == 'squad':
        squad_metric = load("squad_v2")
        def metric_fn(response, example, *args, **kwargs):
            if 'id' in example:
                exid = example['id']
            elif 'id' in example['reference']:
                exid = example['reference']['id']
    ...
    # 支持任意llm_gpt-*新模型
    elif metric.startswith('llm_gpt-'):
